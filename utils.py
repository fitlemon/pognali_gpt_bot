...
async def generate_events_list(userdata) -> dict:
    try:
        ...
        response = await openai.ChatCompletion.acreate(
            model="gpt-3.5-turbo",
            messages=[
                 {"role": "system", "content": f"Ты нейросеть, внедренная в телеграм бот (с названием Погнали, чувак).\
                    ...
                    2. *интересноеместо №2*.\
                    3. и т.д."},
                {"role": "user", "content": f"Куда можно сходить сегодня в моем городе: {today}"}
            ]
        )
